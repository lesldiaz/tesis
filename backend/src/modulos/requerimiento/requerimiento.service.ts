...
export class RequerimientoService extends ServiceGeneral<RequerimientoEntity> {
  ...
  async crearMasivo(
    datosAGuardar: any,
  ): Promise<RespuestaInterface<any> | string> {
    ...
    try {
      if (datosAGuardar.length) {
        ...
        datosAGuardar.forEach(async (requerimiento) => {
          ...
          const requerimientoCreado = await this._requerimientoRepository.save(
            requerimientoGuardar,
          );
          //generar id requerimiento
          requerimientoCreado.idRequerimiento =
            FUNCIONES_GENERALES.generarIdRequerimiento({
              id: requerimientoCreado.id,
              tipoProyecto,
            });
          // editar padre aqui - pendiente
          //editar idReq generado
          const respuestaEditar = await this._requerimientoRepository.update(
            requerimientoCreado.id,
            {
              idRequerimiento: requerimientoCreado.idRequerimiento,
            },
          );
          const actualizacionExitosa: boolean = respuestaEditar.affected > 0;
          ...
          if (tipoProyecto === 'J') {
            ...
            for (let i = 1; i <= 6; i++) {
              if (requerimiento[`bloqueGameplay${i}`]) {
                const bloque = bloquesExistentes.find(
                  (bloque) =>
                    bloque.nombre.toUpperCase() ===
                    requerimiento[`bloqueGameplay${i}`],
                );
                if (bloque) {
            ...
            //guardar bloques
            await this._requerimientoBloqueRepository.save(
              bloquesGameplayAgregar,
            );
          }
    ...
    try {
      if (objeto.length > 1) {
        objeto.forEach(async (requerimiento) => {
          ...
          const proyecto = await this._proyectoRepository.findOne(
  ...
  async editarModoGrafico(objeto): Promise<RespuestaInterface<any> | string> {
    try {
      ...
      delete objeto.proposito;
      console.log(objeto);
      const requerimientoEditado = await this._requerimientoRepository.update(
        objeto.id,
        objeto,
      );
      console.log(requerimientoEditado);
      const actualizacionExitosa: boolean = requerimientoEditado.affected > 0;
      if (!actualizacionExitosa) {
        return new Promise((resolve, reject) =>
      ...
      if (criteriosPaginacion) {
        ...
        atributosRep.forEach((atributo) => {
          if (atributo === 'proyecto') {
            criteriosPaginacion[atributo] = JSON.parse(
              criteriosPaginacion[atributo],
            ); // mandar con comillas el atributo
          } else if (atributo !== 'id') {
            criteriosPaginacion[atributo] = Like(
              `%${criteriosPaginacion[atributo]}%`,
            );
          }
        });
  ...
  async refinamiento(objeto: any): Promise<RespuestaInterface<any> | string> {
    try {
      ...
      const requerimientosARefinar = await this._requerimientoRepository.find({
        ...
        relations: ['proyecto', 'resultado'],
      });
      const requerimientosClonados = JSON.parse(
        JSON.stringify(requerimientosARefinar),
      );
      ...
      requerimientosClonados.forEach(async (requerimientoARefinar) => {
        ...
        if (validacionMin) {
          ...
          observacionesFinales =
            observacionesFinales + 'Características no cumplidas: ';
          ...
          observacionesFinales =
            observacionesFinales + reqNoCumplidos.join(', ');
        }
        const observacion = {

...
export class UsuarioSesionService extends ServiceGeneral<UsuarioSesionEntity> {
  ...
  async crearCustom(
    objeto,
  ): Promise<RespuestaInterface<UsuarioSesionEntity> | string> {
    try {
      ...
      if (usuarioSesionActivo) {
        const sesion = {
          ...
          fechaInicioSesionAnterior:
            usuarioSesionActivo.fechaInicioSesionActual,
          usuario: objeto.usuario,
        };
        const respuestaEditar = await this._usuarioSesionRepository.update(
          usuarioSesionActivo.id,
          sesion,
        );

...
export class UsuarioService extends ServiceGeneral<UsuarioEntity> {
  ...
  async autenticacion(usuario): Promise<UsuarioEntity | string> {
    try {
      ...
      if (encontrar) {
        ...
        return new Promise((resolve) => resolve('Credenciales incorrectas'));
      }
    } catch (e) {
  ...
  async registro(usuario): Promise<UsuarioEntity | string> {
    try {
      ...
      const respuestaUsuarios = await this._usuarioRepository.find({
        nombreUsuario: usuario.nombreUsuario,
      });
      const usuarioNoEncontrado: boolean = respuestaUsuarios.length === 0;
